...
class MyWindowClass(QMainWindow, main_window):
    ...
    def normalWongoMove(self):
        global InnerWongoFolder, OuterWongoFolder, wongo_Folder, in_wongo_folder, out_wongo_folder
        ...
        for rowNum in rowList :
            ...
            wongo_type = str(self.table1.item(rowNum, 1).text()).replace(' ','')
            try:
                ...
                if os.path.isfile(wongo_move_file) :
                    ...
                    if wongo_type == '내부원고' :
                        use_wongo_Folder = InnerWongoFolder
                    elif wongo_type == '외부원고' :
                        use_wongo_Folder = OuterWongoFolder
                    else:
                        errorLogSave('원고종류 >> {} >> wongo_type:{}'.format(wongo_move_file, wongo_type))
                        continue
...
if __name__ == '__main__':
    ...
    try:
        ...
        InnerWongoFolder = config['DEFAULT']['내부사용원고폴더']
        OuterWongoFolder = config['DEFAULT']['외부사용원고폴더']

        #out_wongo_Folder = config['DEFAULT']['외부중복원고'] + "\\"
    except Exception as e:
        try:
            config.read('config_wongo_check_ui.ini', encoding='utf-8-sig')
            wongo_Folder = config['DEFAULT']['중복원고']  # + "\\"
            # use_wongo_Folder  = config['DEFAULT']['사용원고폴더']
            InnerWongoFolder = config['DEFAULT']['내부사용원고폴더']
            OuterWongoFolder = config['DEFAULT']['외부사용원고폴더']
    ...
    #print('InnerWongoFolder : {}'.format(InnerWongoFolder))
    #print('OuterWongoFolder : {}'.format(OuterWongoFolder))
    #sys.exit()
